...
m = ins.metrics(verbose=100)
#t = ins.binaryTarget(metrics=m, target=lambda x: np.sum(x), dimension=100)
t = ins.binaryTarget(metrics=m, target=lambda x: np.sum(x), dimension=500)
...
ga = ins.geneticAlgorithm(target=t, stop=s, popSize=60)
...
ga.opCrossover = ins.shuffled(x1)
...
ga.opMutate = ins.binaryMutation(prob=0.01) 

#ins.decorate(ga, ins.rankIt())

#ins.decorate(ga, [ins.addElite(5), ins.timeIt(ins.timer(m)), ins.rankIt()]) 
